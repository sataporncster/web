					    ...
					    $strSQL =  mysql_query("SELECT * FROM `rehearsal` where `rehearsal`.`status` = 0 ORDER BY `d_update` DESC") or die ("Select Error");
										while($objResult = mysql_fetch_array($strSQL)){
											   $count++;
												...
												echo "<td>".$objResult['contact_tel']."</td>";
												...
												echo "<td>".$objResult['phone']."</td>";
													if($objResult['status']==0){
														...
														echo "<td>ซ่อมเสร็จแล้ว</td>";
													}
												
												echo '<td  align="center" onclick="document.location = \'status_rehearsal.php?id='.$objResult['r_id'].'\';"><img src="../images/paperclip.png" width="32" height="32" title="สถานะการซ่อม"></td>';
												echo "<td  align='center'>"?><a href=detail_rehearsal.php?id=<?=$objResult['r_id']?>><img src='../images/file-word.png' width='32' height='32' title="รายละเอียด"></a><?php "</td>";
												echo "</tr>";
