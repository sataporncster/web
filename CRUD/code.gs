...
function insert_value(request,sheet){
  ...
  var rowData = sheet.appendRow([currentTime,id,nama,gender,alamat,kelas,jurusan,email,kontak]);  
...
var output  = ContentService.createTextOutput();
  ...
  for(var i=1;i<=lr;i++){
    ...
    if(rid==id){
      ...
      sheet.getRange(i,5).setValue(alamat);
      sheet.getRange(i,6).setValue(kelas);
      sheet.getRange(i,7).setValue(jurusan);
      sheet.getRange(i,8).setValue(email);
      sheet.getRange(i,9).setValue(kontak);//////////////////
